...
Write-Host "[1/6] Iniciando user-service (puerto 8081)..." -ForegroundColor Cyan
...
Write-Host "[2/6] Iniciando event-service (puerto 8082)..." -ForegroundColor Cyan
...
Write-Host "[3/6] Iniciando orchestration-service (puerto 8083)..." -ForegroundColor Cyan
Start-Job -Name "orchestration-service" -ScriptBlock {
    Set-Location "d:\Tareas de programacion\SOA\orchestration-service"
    java -jar target\ticket-service-0.0.1-SNAPSHOT.jar 2>&1 | Tee-Object -FilePath "..\logs\orchestration-service.log"
} | Out-Null
Start-Sleep -Seconds 12

Write-Host "[4/6] Iniciando payment-service (puerto 8084)..." -ForegroundColor Cyan
...
Write-Host "[5/6] Iniciando notification-service (puerto 8085)..." -ForegroundColor Cyan
...
Write-Host "[6/6] Iniciando gateway (puerto 8080)..." -ForegroundColor Cyan
...
Write-Host "`nServicios activos: $($ports.Count)/6" -ForegroundColor $(if ($ports.Count -eq 6) { "Green" } else { "Yellow" })
if ($ports.Count -gt 0) {
